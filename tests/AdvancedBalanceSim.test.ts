...
describe('Advanced Balance Simulation', () => {
  it('should simulate varied zombie types and wave progression', () => {
    ...
    for (const tower of scenarios) {
      ...
      for (const wave of waves) {
        ...
        const spawnInterval =
          wave.spawnDuration / wave.zombies.reduce((sum, z) => sum + z.count, 0);
        let spawnTimer = 0;
        const _nextZombieIndex = 0;
        ...
        for (let time = 0; time < waveDuration; time += deltaTime) {
          ...
          if (
            ...
            zombiesSpawned < wave.zombies.reduce((sum, z) => sum + z.count, 0)
          ) {
      ...
      console.log(
        `   Efficiency: ${results.efficiency.toFixed(2)} dmg/$ | Overkill: ${results.avgOverkillPercent.toFixed(1)}%`
      );
      console.log(
        '   Effectiveness vs types:',
        [...damageByZombieType.entries()].map(([k, v]) => `${k}:${v.toFixed(0)}`).join(', ')
      );
    }
    ...
    const earlyGame = allResults.filter(
  ...
  it('should simulate multi-tower synergies', () => {
    ...
    const comboTest = () => {
      ...
      const _waveHp = 30 * 20 + 3 * 200; // swarm + tanks
  ...
  it('should validate tower cost progression', () => {
    ...
    console.log(
      `   Tier 2 avg cost: $${tier2Avg.toFixed(0)} (+${((tier2Avg / tier1Avg - 1) * 100).toFixed(0)}%)`
    );
    console.log(
      `   Tier 3 avg cost: $${tier3Avg.toFixed(0)} (+${((tier3Avg / tier2Avg - 1) * 100).toFixed(0)}%)`
    );
  ...
  it('should simulate late-game horde with 100+ zombies', () => {
    ...
    console.log(
      `   ${hordeWave.zombies.map(z => `${z.count}x ${z.name} (${z.hp} HP)`).join(', ')}\n`
    );
    ...
    const towerTests = [
      {
        type: 'MachineGun',
        dps: TowerConstants.MACHINE_GUN.damage * TowerConstants.MACHINE_GUN.fireRate,
        cost: TowerConstants.MACHINE_GUN.cost,
      },
      {
        type: 'Grenade',
        dps: TowerConstants.GRENADE.damage * TowerConstants.GRENADE.fireRate,
        cost: TowerConstants.GRENADE.cost,
      },
      {
        type: 'Tesla',
        dps: TowerConstants.TESLA.damage * TowerConstants.TESLA.fireRate,
        cost: TowerConstants.TESLA.cost,
      },
      {
        type: 'Sniper',
        dps: TowerConstants.SNIPER.damage * TowerConstants.SNIPER.fireRate,
        cost: TowerConstants.SNIPER.cost,
      },
    ];
    ...
    for (const tower of towerTests) {
      ...
      console.log(
        `   DPS: ${tower.dps.toFixed(1)} (x${areaMultiplier} area = ${effectiveDPS.toFixed(1)})`
      );
      console.log(
        `   Est. kills: ${zombiesKilled}/${totalZombies} (${((zombiesKilled / totalZombies) * 100).toFixed(0)}%)`
      );
